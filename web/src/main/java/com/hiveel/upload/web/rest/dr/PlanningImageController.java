...
@RestController("planningImageControllerDr")
@RequestMapping("/dr")
public class PlanningImageController {
    ...
    public Rest<String> image(@RequestAttribute("loginAccount") Account loginAccount, PlanningImage planningImage) throws ParameterException, IOException {
        ...
        if (planningImage.getSaveFolder().equals("PERSON")) {
            planningImage.setSaveFor(loginAccount.getExtra() + File.separator + loginAccount.getId());
        }
