...
public class PlanningDeleteControllerTest {
    ...
    public void delete() throws Exception {
        ...
        String fileName = "ex0006";
        String saveFolder = "ISSUE";
        MvcResult mvcResult = this.mvc.perform(post("/api/image").header("apiKey", "hiveel")
                .param("base64", base64)
        ...
        //请求删除
        this.mvc.perform(MockMvcRequestBuilders.delete("/api/file").header("apiKey", "hiveel")
                .param("saveFolder", saveFolder)
                .param("fileName", rest.getData()))
                .andExpect(status().isOk())
                .andReturn();
        //验证删除结果，文件应该不再存在
        Assert.assertEquals(false,uploaded.exists());
    }
