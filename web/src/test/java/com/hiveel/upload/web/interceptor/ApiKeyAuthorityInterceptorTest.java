...
@SpringBootTest
@ActiveProfiles("test")
public class ApiKeyAuthorityInterceptorTest {
    private MockMvc mvc;
    @Autowired
    ...
    public void apiTest() throws Exception {
        //apiKey正确可以通过拦截器
        String result = mvc.perform(MockMvcRequestBuilders.post("/api/image")
                .header("apiKey", "hiveel")
                .param("base64", base64)
        ...
        //apikey错误不能通过拦截器访问接口
        result = mvc.perform(MockMvcRequestBuilders.post("/api/image")
                .header("apiKey", "hiveelErr")
                .param("base64", base64)
                .param("saveFolder", saveFolder)
                .param("saveFor", "uxx123")
                .param("fileNameWithoutType", fileName)
