package com.hiveel.upload.web.rest.dr;
...
@SpringBootTest
@ActiveProfiles("test")
public class PlanningImageControllerTest {
    private MockMvc mvc;
    @Autowired
    private WebApplicationContext context;

    @Test
    public void imagePerson() throws Exception {
        ...
        String saveFolder = "PERSON";
        Account account = new Account.Builder().set("id",1L).set("extra","DR").build();
        Mockito.when(authManager.findAccountByToken(Mockito.anyString())).thenReturn(account);
        MvcResult mvcResult = this.mvc.perform(post("/dr/image").header("authorization", "Bearer 123456778")
                .param("base64", base64)
